...
module.exports = new Event('interactionCreate', async (client, interaction) => {
	...
	if(interaction.isApplicationCommand() && !interaction.user.bot && interaction.guild) {
		...
		if (!interaction.member.permissionsIn(interaction.channel).has(command.permission)) {return interaction.reply('You don\'t have permission to run this command!');}
	...
	if(interaction.componentType === 'BUTTON' && interaction.customId.includes('buttoncontrol')) {
		...
		if(!queue || !queue.playing || !interaction.member.voice.channelId || (interaction.guild.me.voice.channelId && interaction.member.voice.channelId !== interaction.guild.me.voice.channelId)) {return;}
